...
const ActionLogo = ({ post }) => {
    ...
    const handleLikeAndUnlike = async () => {
        ...
        try {
            ...
            if (!liked) {
                //add the id of the current user to the likes array of the post
                ...
                const updatedPosts = posts.map((p) => {
                    if(p._id === post._id){
                        return  { ...p, likes: p.filter((id) => id !== user._id)};
                    }
                    return p;
                });
                setPosts(updatedPosts);
    ...
    const handleReply = async () => {
        if (!user) {
            return showToast('Error', 'You need to be logged in to reply to a post', 'error');
        }
        if (reply.trim() === "") {
